/* ============================================================================
   ...
   It does two jobs:
     ...
     1. Keeps a copy of your homework so it can check reminders without the app
...
async function check(allowWhileVisible) {
  ...
  for (const t of tasks) {
    ...
    try {
      await self.registration.showNotification(t.title || 'Homework reminder', {
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async () => {
    try {
      ...
      if (hit) return hit;
      return new Response('Offline — reopen School Memory when you have signal.', {
